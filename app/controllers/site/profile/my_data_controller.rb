class Site::Profile::MyDataController < Site::ProfileController

  before_action :set_profile_member, only: [:edit, :update]

  def edit
  end

  def update
    if @profile_member.update(params_profile_memeber)
      redirect_to edit_site_profile_my_datum_path(current_member), notice: "Dados Atualizados!"
    else
      render :edit
    end
  end

  private

  def set_profile_member
    @profile_member = ProfileMember.find_or_create_by(member_id: current_member.id)
  end

  def params_profile_memeber
    params.require(:profile_member).permit(:first_name, :second_name, :birthdate)
  end

end
